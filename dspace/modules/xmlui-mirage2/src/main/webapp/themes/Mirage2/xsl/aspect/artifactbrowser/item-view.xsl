...
<xsl:stylesheet
        ...
        <div class="item-summary-view-metadata">
            ...
            <div class="row">
                <div class="col-sm-4">
                    ...
                    <xsl:call-template name="impact-scopus"/>
                    </xsl:if>
    ...
    <xsl:template name="itemSummaryView-DIM-contents">
        <xsl:if test="dim:field[@element='description' and @qualifier='tableofcontents']">
            <div class="simple-item-view-description item-page-field-wrapper table">
                <h5><i18n:text>xmlui.dri2xhtml.METS-1.0.item-contents</i18n:text></h5>
                <div>
        ...
        <xsl:if test="dim:field[@element='contributor'][@qualifier='author' and descendant::text()] or dim:field[@element='creator' and descendant::text()] or dim:field[@element='contributor' and descendant::text()]">
            <div class="simple-item-view-authors item-page-field-wrapper table">
                <xsl:variable name="header">
                    <xsl:choose>
                        <xsl:when test="dim:field[@element='contributor'][not(@qualifier='editor') and descendant::text()]">
    ...
    <xsl:template name="itemSummaryView-DIM-editors-entry">
        <div>
            ...
            <a>
                <xsl:attribute name="href">
                    <xsl:text>/discover?filtertype=editor&amp;filter_relational_operator=equals&amp;filter=</xsl:text>
                    <xsl:copy-of select="node()"/>
                    <xsl:text>&amp;sort_by=dc.date.issued_dt&amp;order=desc</xsl:text>
                </xsl:attribute>
                <xsl:copy-of select="node()"/>
            ...
            <div class="simple-item-view-uri item-page-field-wrapper table">
                ...
                <span>
                    <xsl:for-each select="dim:field[@element='relation' and @qualifier='hasversion']">
                        ...
                        <a>
                            <xsl:attribute name="href">
                                <xsl:value-of select="substring-after(.,'10862/')"/>
                            </xsl:attribute>
   ...
   <xsl:template match="mets:fileGrp[@USE='LICENSE']">
    ...
    <xsl:template name="impact-scopus">
            ...
            <p id="scopus" class="hidden">
                ...
                <small>Cited </small>
                <a href="to be filled by impact.js" target="_blank">
                    <!-- <img src="{concat($theme-path,'/images/scopus.png')}"
                         i18n:attr="alt"
                         alt="xmlui.mirage2.itemSummaryView.ImpactSection.ScopusLogoAlt" />-->
                    <span class="citation-count badge" title="View citing articles in Scopus"></span>
                </a>
                <small> times in
                    <a class="scopus-record" target="_blank" title="View record in Scopus">Scopus</a>
                </small>
            </p>
    </xsl:template>
