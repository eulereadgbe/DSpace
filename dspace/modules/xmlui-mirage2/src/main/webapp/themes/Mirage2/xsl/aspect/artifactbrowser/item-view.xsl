...
<xsl:stylesheet
        ...
        <div class="item-summary-view-metadata">
            ...
            <div class="row">
                <div class="col-sm-4">
					...
					<xsl:comment>DOI: <xsl:value-of select='$identifier_doi'/></xsl:comment>
    ...
    <xsl:template name="itemSummaryView-DIM-subject">
        <xsl:if test="dim:field[@element='subject'][@qualifier='asfa'] or dim:field[@element='subject' and not(@qualifier)]
            or dim:field[@element='subject'][@qualifier='lcsh'] or dim:field[@element='coverage'][@qualifier='spatial']">
            <div class="item-page-field-wrapper table">
                ...
                <div>
                    <xsl:for-each select="dim:field[@element='subject'][@qualifier='asfa']">
                        ...
                        <xsl:call-template name="itemSummaryView-DIM-ASFA-entry"/>
                        <xsl:if test="count(following-sibling::dim:field[@element='subject'][@qualifier='asfa']) != 0">
                            <br/><!-- <xsl:text>; </xsl:text> -->
                        </xsl:if>
                    </xsl:for-each>
                    <xsl:if test="(dim:field[@mdschema='dc' and @element='subject' and not(@qualifier)])">
                        <xsl:if test="count(dim:field[@element='subject'][@qualifier='asfa']) != 0">
                            <br/><!-- <xsl:text>; </xsl:text> -->
                        </xsl:if>
                        <xsl:for-each select="dim:field[@mdschema='dc' and @element='subject' and not(@qualifier)]">
                            <a class="label label-primary">
                                <xsl:attribute name="href">
                                    <xsl:value-of
                                            select="concat($context-path,'/discover?filtertype=')"/>
                            ...
                            <xsl:if test="count(following-sibling::dim:field[@mdschema='dc' and @element='subject' and not(@qualifier)]) != 0">
                                <br/><!-- <xsl:text>; </xsl:text> -->
                            </xsl:if>
                        </xsl:for-each>
                    </xsl:if>
                    <xsl:if test="dim:field[@element='subject'][@qualifier='lcsh'][@authority]">
                        <xsl:if test="count(dim:field[@element='subject'][@qualifier='asfa']) != 0
                        or count((dim:field[@mdschema='dc' and @element='subject' and not(@qualifier)])) != 0">
                            <br/><!-- <xsl:text>; </xsl:text> -->
                        </xsl:if>
                        <xsl:for-each select="dim:field[@element='subject'][@qualifier='lcsh'][@authority]">
                            <a class="label label-primary">
                                <xsl:attribute name="href">
                                    <xsl:value-of
                                            select="concat($context-path,'/discover?filtertype=')"/>
                    ...
                    <xsl:if test="dim:field[@element='coverage'][@qualifier='spatial']">
                        <xsl:if test="count(dim:field[@element='subject'][@qualifier='asfa']) != 0
                        or count((dim:field[@mdschema='dc' and @element='subject' and not(@qualifier)])) != 0 or count(dim:field[@element='subject'][@qualifier='lcsh']) !=0">
                            <br/><!-- <xsl:text>; </xsl:text> -->
                        </xsl:if>
    ...
    <xsl:template name="itemSummaryView-DIM-AGROVOC-entry">
        <xsl:if test="contains(@authority,'agrovoc')">
            <xsl:text>&#160;</xsl:text>
            <a>
                <xsl:attribute name="href">
    ...
    <xsl:template name="itemSummaryView-DIM-ASFA-entry">
        <xsl:if test="contains(@authority,'agrovoc')">
            <xsl:text>&#160;</xsl:text>
            <a>
                <xsl:attribute name="href">
                    <xsl:text>https://agrovoc.fao.org/skosmosAsfa/asfa/en/page?uri=</xsl:text>
                    <xsl:value-of select="@authority"/>
                </xsl:attribute>
                <xsl:attribute name="target">
                    <xsl:text>_blank</xsl:text>
                </xsl:attribute>
                <img src="{$theme-path}images/AGROVOC-logo.gif" alt="ASFA" class="vocabulary" title="ASFA"/>
            </a>
        </xsl:if>
    </xsl:template>
    ...
    <xsl:template name="itemSummaryView-DIM-KEYWORDS">
        <xsl:if test="dim:field[@element='subject' and @mdschema='local' and not(@qualifier) and descendant::text()]">
                <div class="simple-item-view-issn item-page-field-wrapper table">
                    <h5>Keywords</h5>
                    <span>
                        <xsl:for-each select="dim:field[@mdschema='local' and @element='subject' and not(@qualifier) and descendant::text()]">
                            ...
                            <xsl:if test="count(following-sibling::dim:field[@mdschema='local' and @element='subject' and not(@qualifier)]) != 0">
                                <xsl:text> </xsl:text>
                            </xsl:if>
                        </xsl:for-each>
                    </span>
   ...
   <xsl:template match="mets:fileGrp[@USE='LICENSE']">
    ...
    <xsl:template name="email">
        <a>
            <xsl:attribute name="href">
                <xsl:text>https://docs.google.com/forms/d/e/1FAIpQLSdD5EeftsJil_NSyhJ0yNOyTGPwgngQ5E0Cj6V5IV-hd9jYZA/viewform?</xsl:text>
                <xsl:text>entry.366404937=</xsl:text>
                <xsl:value-of select="dim:field[@element='title'][not(@qualifier)][1]/text()" disable-output-escaping="yes"/>
                <xsl:text>&amp;entry.1117290746=</xsl:text>
            ...
            <div id='altmetric'
                ...
                <xsl:choose>    <!-- data-doi data-handle data-arxiv-id data-pmid -->
                    ...
                    <xsl:when test='$identifier_handle'>
                        <xsl:attribute name='data-handle'><xsl:value-of select='substring-after($handle-uri,"http://hdl.handle.net/")'/></xsl:attribute>
                    </xsl:when>
                </xsl:choose>
                &#xFEFF;
            </div>
    ...
    <xsl:template name="impact-dimensions">
        ...
        <span>
            <xsl:attribute name="class">
                <xsl:text>__dimensions_badge_embed__</xsl:text>
            </xsl:attribute>
            <xsl:attribute name='data-doi'><xsl:value-of select='dim:field[@element="identifier" and @qualifier="doi" and descendant::text()]'/></xsl:attribute>
            <xsl:attribute name="data-hide-zero-citations"><xsl:text>true</xsl:text></xsl:attribute>
            <xsl:attribute name="data-legend"><xsl:text>always</xsl:text></xsl:attribute>
            <xsl:attribute name="data-style"><xsl:text>small_circle</xsl:text></xsl:attribute>
        </span>
        </div>
    </xsl:template>
