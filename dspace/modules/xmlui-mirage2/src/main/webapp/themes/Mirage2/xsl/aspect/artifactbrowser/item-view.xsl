...
<xsl:stylesheet
        ...
        <div class="item-summary-view-metadata">
            ...
            <div class="row">
                <div class="col-sm-4">
                    ...
                    <xsl:call-template name="itemSummaryView-DIM-control-number"/>
                </div>
                <div class="col-sm-8">
                    <xsl:call-template name="itemSummaryView-DIM-abstract"/>
                    <xsl:call-template name="itemSummaryView-DIM-extract"/>
                ...
                <xsl:otherwise>
                    <img class="img-thumbnail" alt="Thumbnail">
                        <xsl:attribute name="data-src">
                            ...
                            <xsl:text> \n page </xsl:text>
                            <xsl:value-of select="//mets:dmdSec/mets:mdWrap[@OTHERMDTYPE='DIM']/mets:xmlData/dim:dim/dim:field[@element='citation' and @qualifier='spage']/text()"/>
                        </xsl:attribute>
                    </img>
                </xsl:otherwise>
            </xsl:choose>
    ...
    <xsl:template name="itemSummaryView-DIM-extract">
        <xsl:if test="dim:field[@element='seafdecaqd' and @qualifier='extract']">
            <div class="simple-item-view-description item-page-field-wrapper table">
                <h5><i18n:text>xmlui.dri2xhtml.METS-1.0.item-extract</i18n:text></h5>
                <div>
                    <xsl:for-each select="dim:field[@element='seafdecaqd' and @qualifier='extract']">
                        <xsl:choose>
                            <xsl:when test="node()">
                                <xsl:copy-of select="node()"/>
                            </xsl:when>
                        ...
                        <xsl:if test="count(following-sibling::dim:field[@element='seafdecaqd' and @qualifier='extract']) != 0">
                            <div class="spacer">&#160;</div>
                        </xsl:if>
                    </xsl:for-each>
                    <xsl:if test="count(dim:field[@element='seafdecaqd' and @qualifier='extract']) &gt; 1">
                        <div class="spacer">&#160;</div>
                    </xsl:if>
                </div>
            </div>
        ...
        <div>
            ...
            <a>
                <xsl:attribute name="href">
                    ...
                    <xsl:text>author&amp;filter_relational_operator=equals&amp;filter=</xsl:text>
                    <xsl:copy-of select="node()"/>
                    <xsl:text>&amp;sort_by=dc.date.issued_dt&amp;order=desc</xsl:text>
                </xsl:attribute>
                <xsl:copy-of select="node()"/>
            </a>
        </div>
    ...
    <xsl:template name="itemSummaryView-DIM-LINK">
        <xsl:if test="dim:field[@element='relation' and @qualifier='uri' and descendant::text()]">
            <div class="simple-item-view-uri item-page-field-wrapper table">
                <h5><i18n:text>xmlui.dri2xhtml.METS-1.0.item-link</i18n:text></h5>
                <span>
            ...
            <div class="simple-item-view-date word-break item-page-field-wrapper table">
                ...
                <xsl:for-each select="dim:field[@element='date' and @qualifier='issued']">
                    ...
                    <xsl:if test="string-length(./node()) = 10">
                        <xsl:copy>
                            <xsl:call-template name="format_date">
                                <xsl:with-param name="date" select="substring(./node(),1,10)"/>
                            </xsl:call-template>
                        </xsl:copy>
                    </xsl:if>
                    <xsl:if test="string-length(./node()) != 10">
    ...
    <xsl:template name="itemSummaryView-DIM-control-number">
        <xsl:if test="dim:field[@element='seafdecaqd' and @qualifier='controlnumber' and descendant::text()]">
            <div class="simple-item-view-show-full item-page-field-wrapper table">
                <h5>
                    <i18n:text>xmlui.dri2xhtml.METS-1.0.item-control-number</i18n:text>
                </h5>
                <xsl:value-of select="dim:field[@element='seafdecaqd'][@qualifier='controlnumber']"/>
            </div>
        </xsl:if>
    </xsl:template>
    ...
    <xsl:template name="itemSummaryView-DIM-file-section">
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:call-template name="request-document"/>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
   ...
   <xsl:template match="mets:fileGrp[@USE='LICENSE']">
    ...
    <xsl:template name="format_date">
        ...
        <xsl:text> </xsl:text><xsl:value-of select="number(substring($date, 9, 2))"/><xsl:text>, </xsl:text>
        <xsl:value-of select="substring($date, 1, 4)"/>

    </xsl:template>

    <xsl:template name="request-document">
        <xsl:if test="not(dim:field[@element='relation' and @qualifier='uri' and descendant::text()])">
            <div class="simple-item-view-uri item-page-field-wrapper table">
                <h5>
                    <i18n:text>xmlui.dri2xhtml.METS-1.0.item-files-viewOpen</i18n:text>
                </h5>
                <span>
                    <a>
                        <xsl:variable name="mail-subject">
                            <xsl:text>&amp;subject=Request%20copy%20of%20document%20from%20ANI&amp;</xsl:text>
                        </xsl:variable>
                        <xsl:variable name="item">
                            <xsl:choose>
                                <xsl:when test="dim:field[@element='identifier'][@qualifier='citation']">
                                    <!-- <xsl:value-of select="dim:field[@element='identifier'][@qualifier='citation'][1]/node()"/> -->
                                    <xsl:call-template name="string-replace-all">
                                        <xsl:with-param name="text"
                                                        select="dim:field[@element='identifier'][@qualifier='citation'][1]/node()"/>
                                ...
                                <xsl:otherwise>
                                    <xsl:call-template name="string-replace-all">
                                        <xsl:with-param name="text"
                                                        select="dim:field[@element='title'][not(@qualifier)][1]/text()"/>
                                        <xsl:with-param name="replace" select="'&amp;'"/>
                                        <xsl:with-param name="by" select="'%26'"/>
                                    </xsl:call-template>
                                </xsl:otherwise>
                            ...
                            <xsl:text>" (</xsl:text>
                            <xsl:value-of select="dim:field[@element='identifier'][@qualifier='uri'][1]/node()"/>
                            <xsl:text>)</xsl:text>
                        </xsl:variable>
                        <xsl:variable name="mail-body">
                            <xsl:text>body=To%20Whom%20It%20May%20Concern:%0A%0APlease%20supply%20the%20document:%20%22</xsl:text>
                            <xsl:value-of select="$item"/>
                            <xsl:text>%0A%0AI%20will%20use%20the%20requested%20materials%20only%20for%20private%20study,%20scholarship%20or%20research.%20I%20understand%20that%20use%20for%20any%20other%20purpose%20may%20require%20the%20authorization%20of%20the%20copyright%20owner.%20I%20accept%20responsibility%20for%20determining%20if%20copyright%20owner%20authorization%20is%20required%20and%20for%20obtaining%20authorization%20when%20it%20is%20required.</xsl:text>
                        </xsl:variable>
                        <xsl:attribute name="href">
                        ...
                        <small><i aria-hidden="true">
                            <xsl:attribute name="class">
                                <xsl:text>glyphicon glyphicon-envelope</xsl:text>
                            </xsl:attribute>
                        </i></small>
                        <xsl:text> </xsl:text>
                        <xsl:text>Request this article</xsl:text>
                    </a>
                </span>
            </div>
        </xsl:if>
