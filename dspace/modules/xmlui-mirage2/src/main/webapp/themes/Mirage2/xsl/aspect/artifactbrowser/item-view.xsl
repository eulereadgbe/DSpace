...
<xsl:stylesheet
    ...
    <xsl:template name="itemSummaryView-DIM-keyword">
        <xsl:if test="dim:field[@element='subject'][@qualifier='asfa'] or dim:field[@mdschema='local'][@element='subject' and not(@qualifier)]
        or dim:field[@element='subject'][@qualifier='scientificname'] or dim:field[@element='subject'][@qualifier='mesh']
or dim:field[@element='coverage'][@qualifier='spatial']">
            <div class="item-page-field-wrapper table">
                <h5><i18n:text>xmlui.dri2xhtml.METS-1.0.item-keyword</i18n:text></h5>
                <div>
                    <xsl:for-each select="dim:field[@mdschema='local'][@element='subject' and not(@qualifier)]">
                        <a>
                            <xsl:attribute name="href">
                                <xsl:value-of
                                        select="concat($context-path,'/discover?filtertype=')"/>
                        ...
                        <xsl:if test="count(following-sibling::dim:field[@mdschema='local'][@element='subject' and not(@qualifier)]) != 0">
                            <xsl:text>; </xsl:text>
                        </xsl:if>
                    </xsl:for-each>
                    <xsl:if test="dim:field[@mdschema='local'][@element='subject'][@qualifier='scientificname']">
                        <xsl:if test="count(dim:field[@mdschema='local'][@element='subject' and not(@qualifier)]) != 0">
                            <xsl:text>; </xsl:text>
                        </xsl:if>
                    ...
                    <xsl:if test="dim:field[@element='subject'][@qualifier='mesh']">
                        ...
                        or count(dim:field[@mdschema='local'][@element='subject' and not(@qualifier)]) != 0">
                            <xsl:text>; </xsl:text>
                        </xsl:if>
