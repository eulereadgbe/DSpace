...
public class DocumentDeliveryForm extends AbstractDSpaceTransformer implements CacheableProcessingComponent
{
    ...
    public Serializable getKey() {
        ...
        return HashUtil.hash(requesterName + "-" + requesterEmail + "-" + purpose + "-" + page + "-" + title + "-"
                + userType + "-" + userTypeOther + "-" + organizationType + "-" +  "-" + organizationTypeOther + "-" + institution + "-" + userAddress + "-"+ agreement);
    }
            ...
            UIException, SQLException, IOException, AuthorizeException
    {
        ...
        //documentdelivery.addPara(T_para1.parameterize(parameters.getParameter("handle","unknown")));
        
        List form = documentdelivery.addList("form",List.TYPE_FORM);

        Text email = form.addItem().addText("requesterEmail");
        email.setAutofocus("autofocus");
        email.setLabel(T_email);
        email.setHelp(T_email_help);
        email.setValue(parameters.getParameter("requesterEmail",""));
        ...
        documentdelivery.addHidden("handle").setValue(handle);
    }
}
