...
public class CitationDocument {
    ...
    private void generateCoverPage(Document cDoc, PdfWriter writer, CitationMeta cMeta) throws DocumentException {
        ...
        Paragraph university = new Paragraph("SEAFDEC/AQD Institutional Repository", helv11_bold);
        cDoc.add(university);

        PdfPTable repositoryTable = new PdfPTable(2);
        repositoryTable.setWidthPercentage(100);

        Chunk repositoryName =  new Chunk("SAIR", helv11_bold);
        PdfPCell nameCell = new PdfPCell();
        nameCell.setBorderWidth(0);
        nameCell.addElement(repositoryName);

        Chunk repositoryURL =   new Chunk("repository.seafdec.org.ph", helv11_bold);
        repositoryURL.setAnchor("http://repository.seafdec.org.ph");
        ...
        Paragraph creators = new Paragraph(getAllMetadataSeperated(item, "dc.contributor.author"), helv16);
       ...
       try {
        ...
        Paragraph license = new Paragraph("Downloaded from SAIR, SEAFDEC/AQD's Institutional Repository", helv12_italic);
